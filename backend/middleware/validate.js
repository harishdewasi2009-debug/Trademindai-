...
const validateMarketQuotes = [
  query('symbols')
    ...
    // FIX (root cause of "Screener shows no quote" / NSE+BSE+MCX quotes all
    // silently failing): marketController.js's getQuotes/getSignals accept
    // a per-entry "SYMBOL:EXCHANGE" override (e.g. "RELIANCE:BSE_EQ") so a
    // dual-listed symbol's NSE row and BSE row (and any MCX row) each
    // resolve against their own exchange instead of colliding. But this
    // regex was never updated when that syntax was added — it has no ':'
    // in its character class. Since the Screener ALWAYS tags every row
    // with its exchange (screenerExchange item.exchange is never empty),
    // essentially every real /api/market/quotes and /api/market/signals
    // call from the Screener, the search dropdowns, and the stock-detail
    // modal has been sending a colon and getting rejected with 400
    // "symbols contains invalid characters" — caught by the frontend's
    // `.catch(()=>({}))` and silently turned into "No quote yet" for every
    // row, on every exchange, instead of a visible error. Plain
    // comma-separated symbols with no exchange (the ticker's TICKER_SYMBOLS
    // list) never hit this because they never contained a colon — which is
    // why the ticker looked fine while the Screener didn't.
